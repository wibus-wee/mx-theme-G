...
const Ttwindow: FC<ITwindow> = (props: ITwindow = {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (!wrapRef.current || props.duration === Infinity) { // 如果没有传入duration，或者传入的duration为Infinity，则不会自动关闭
        return
      }
  ...
  return (
    ...
    >
      <div className={clsx(styles.title)}>
        ...
        <div>{props.title}</div>
        {
          props.image && <img src={props.image} alt={props.title} className={styles.image} />
        }
        {
          props.allowClose && <div className={styles.close} onClick={() => {
            wrapRef.current?.classList.add(styles['hide'])
          }}>
      ...
      <div className={clsx(styles.text)}>
        ...
        {
          props.href && (
            <a href={props.href} target={props.target} className={clsx(styles.link)}>
...
export const TwindowContainer = () => {
  return (
    <div id="twindow-container" className={clsx(styles['twindow-container'])}></div>
  )
}
